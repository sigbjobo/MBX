...
void GetCloseTrimerImage(std::vector<double> box, std::vector<double> box_inv, size_t nat1, size_t nat2, size_t nat3,
  ...
  void GetCloseTetramerImage(std::vector<double> box, std::vector<double> box_inv, size_t nat1, size_t nat2, size_t nat3,  size_t nat4,   size_t nt, std::vector<double> &xyz1, std::vector<double> &xyz2, std::vector<double> &xyz3, std::vector<double> &xyz4) {
	...
	// Move monomer 4 to be the closest image to mon1 if needed
        xr0 = box_inv[0] * xyz4[shift4] + box_inv[3] * xyz4[shift4 + 1] + box_inv[6] * xyz4[shift4 + 2];
        yr0 = box_inv[1] * xyz4[shift4] + box_inv[4] * xyz4[shift4 + 1] + box_inv[7] * xyz4[shift4 + 2];
        zr0 = box_inv[2] * xyz4[shift4] + box_inv[5] * xyz4[shift4 + 1] + box_inv[8] * xyz4[shift4 + 2];

        dx0 = std::floor(xr0 - x_rec + 0.5);
        dy0 = std::floor(yr0 - y_rec + 0.5);
        dz0 = std::floor(zr0 - z_rec + 0.5);

        for (size_t j = 0; j < nat4; j++) {
            double xr = box_inv[0] * xyz4[shift4 + 3 * j] + box_inv[3] * xyz4[shift4 + 3 * j + 1] +
                        box_inv[6] * xyz4[shift4 + 3 * j + 2];
            double yr = box_inv[1] * xyz4[shift4 + 3 * j] + box_inv[4] * xyz4[shift4 + 3 * j + 1] +
                        box_inv[7] * xyz4[shift4 + 3 * j + 2];
            double zr = box_inv[2] * xyz4[shift4 + 3 * j] + box_inv[5] * xyz4[shift4 + 3 * j + 1] +
                        box_inv[8] * xyz4[shift4 + 3 * j + 2];

            xr -= dx0;
            yr -= dy0;
            zr -= dz0;

            xyz4[shift4 + 3 * j + 0] = box[0] * xr + box[3] * yr + box[6] * zr;
            xyz4[shift4 + 3 * j + 1] = box[1] * xr + box[4] * yr + box[7] * zr;
            xyz4[shift4 + 3 * j + 2] = box[2] * xr + box[5] * yr + box[8] * zr;
        }
  ...
  void GEN ( int n, int k, int root, int level, int reminder, int m,
	     	    my_kd_tree_t index, int *Index, int **subgraph, int **childSet,double cutoff);
  void NEG ( int n, int k, int root, int level, int reminder, int m,
	     	    my_kd_tree_t index, int *Index, int **subgraph, int **childSet,double cutoff);

void initChildSet(int root, int level, int **subgraph, int **childSet,
		    kdtutils::PointCloud<double> ptc, my_kd_tree_t index, int nmon2, double cutoff) {
    register int *N;
    register int i, j;
    ...
    for(i = 1; i <= parents[0]; i++) {
      ...
      const size_t nMatches = index.radiusSearch(ptc[i], cutoff * cutoff, ret_matches, params);
      N[0]=nMatches;
      
      //fix indices
      for (size_t j = 0; j < nMatches; j++) {
	size_t pos = ret_matches[j].first / nmon2;
	N[j+1]=ret_matches[j].first - nmon2 * pos;
      }
  ...
  void Explore(int root, int level, int reminder,
	       my_kd_tree_t index, int **subgraph, bool *Visited, int **Index, int **childSet,double cutoff) {
    ...
    initChildSet(root, level, subgraph, childSet, 
		 ptc, index,  nmon2,  cutoff); 
	...
	Index[level][i] = i;
      ...
      Explore(root, level + 1, reminder - k,
	      index, subgraph, Visited, Index, childSet,cutoff);
      
      GEN( childSet[level][0], k, root, level, reminder, k,
	   index, Index, subgraph, childSet, Visited); 	
  ...
  void swap(int i, int j, int root, int level, int reminder, int m,
	    my_kd_tree_t index, int *Index, int **subgraph, int **childSet,bool *Visited,double cutoff) {
  ...
  void GEN( int n, int k, int root, int level, int reminder, int m,
    ...
    if (k > 0 && k < n) {
      GEN( n-1, k, root, level, reminder, m,
	   index,Index, subgraph, childSet,Visited,cutoff);
      if (k == 1) 
  ...
  void NEG( int n, int k, int root, int level, int reminder, int m,
	    my_kd_tree_t index, int *Index, int **subgraph, int **childSet,bool *Visited) {	
    if (k > 0 && k < n) {
  ...
  void Enumerate( int **subgraph, int subgraphSize,
		  bool *Visited, int nmon, int **Index, int **childSet, my_kd_tree_t index,double cutoff) {
    register int v;
